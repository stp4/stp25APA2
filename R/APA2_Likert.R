...
APA2.likert <- function(x,
                        ...
                        labels = c("low", "neutral", "high"),
                        order=FALSE) {
    # neue Likert-Funktion
RowSums2<-function(x) if( is.vector(x) ) x  else rowSums(x, na.rm=TRUE)

if(!is.null(ReferenceZero)){
    if(is.character(ReferenceZero)) ReferenceZero <- which(x$levels %in% ReferenceZero)
    else if(!is.numeric(ReferenceZero)) ReferenceZero <- median(1:nlevels)
    #else {}
    cat(ReferenceZero, "\\n")

    if(ceiling(ReferenceZero) == floor(ReferenceZero) ){
            freq <- cbind(lowrange = RowSums2( x$freq[,1:(ReferenceZero-1)] ),
                          neutral = x$freq[,ReferenceZero],
                          highrange= RowSums2( x$freq[,(ReferenceZero+1) : nlevels] ))
            colnames(freq) <- c( paste0(labels[1],"(1:",ReferenceZero-1,")"),
                                 paste0(labels[2],"(",ReferenceZero,")"),
                                 paste0(labels[3],"(", ReferenceZero+1,":", nlevels, ")"))
            x$freq <- freq
            x$freq.na<- if( names(x$freq.na)[ncol(x$freq.na)] == "NA" )
                cbind(freq, x$freq.na[ncol(x$freq.na)] ) else freq

    }else{
            freq <- cbind( lowrange = RowSums2( x$freq[,1:floor(ReferenceZero)] ),
                           highrange= RowSums2( x$freq[,ceiling(ReferenceZero):nlevels] ))
            colnames(freq) <- c( paste0(labels[1],"(1:",floor(ReferenceZero),")"),
...
ans<- if(print.mean) cbind(x$names,
                               x$freq,
                               n=x$n,
                               Mittelwert=x$Mittelwert)
    else  cbind(x$names, x$freq)

    if (!is.logical(order) & order=="decreasing" ) ans<- ans[order(x$m,  decreasing=TRUE), ]
    else if(order) ans<- ans[order(x$m),]
    else(NULL)
