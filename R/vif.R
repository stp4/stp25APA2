...
#' @title VIF Funktion
#' @name VIF
#' @description  variance inflation factor, aka VIF  values over 5 are troubling.
...
#' @rdname VIF
#' @export
VIF2 <- function(..., caption = "VIF") {
  fits <- list(...)
  for (x in fits) {
    res <- data.frame(VIF(x, ...))
    names(res) <- "VIF"

    Output(cbind(Source = rownames(res), res),
